...
import main.java.ru.brown.service.util.MD5Util;
...
public class DbUser {
     ...
     * In accordance with Gravatar's requirements the email will be hashed with the MD5 hash and returned as part of the url.
     * The url will also include the s=xx attribute to request a Gravatar of a particular size.
     ...
     * Pass the method a Bhuser with all the values set to your liking and this method will update the database with these values.
     * This method doesn't actually return anything but the good feeling that your update has been completed. If it can't be completed then it won't tell you.
     * Sounds like something needs to be added in the future. Hmmm.
     *
     * @param bhUser
     */
     ...
     * Not sure why you'd want to delete a Bhuser from the database but this method will do it for you.
     * This method does not explicitly remove the user's posts, but most likely you've set up the database
     * with cascading deletes, which will take care of that. Gives no feedback.
     *
     ...
     * You've got the email when they log in but you really need the user and all its related information.
     * This method will find the user matching that email.
     * The database should ensure that you can't have two users with the same email.
     * Otherwise there's no telling what you'd get.
     *
     * @param email
     ...
     * Checks the database and counts the number of users with this username and password.
     * If it returns 0 then either the username or password don't exist in the database.
     * If it returns 1 then you have found the user with that username and password.
     * If it returns >1 then you need to fix your database.
     *
